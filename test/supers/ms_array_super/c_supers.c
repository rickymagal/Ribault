 ...
 *
 * Super index mapping (from codegen):
 *   s4 = merge_pair  — input: C-list [lptr, rptr]; output: merged ptr
 *   s5 = sort_leaf   — input: C-list [ptr, size];  output: arr_alloc'd ptr
 *   s6 = init_super  — input: C-list [N];          output: ptr (raw, no embed)
 *
