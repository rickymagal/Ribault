...
"valtag":       31,
"super":        32,
"superi":       32, #super and superi have the same opcode, super's immed is set to 0   
"specsuper":    32 | (1 << (SIZEOF('i')*8 - SIZE_OF_SRC_LEN - SIZE_OF_DST_LEN - 1)),
"specsuperi":   32 | (1 << (SIZEOF('i')*8 - SIZE_OF_SRC_LEN - SIZE_OF_DST_LEN - 1)) #speculative instructions have the most significant bit of the opcode part set to 1.
}
...
class ALU2i(Instruction):
        def __init__(self, tks, assembler):
                self.src1 = assembler.getsources(tks[2])
                self.src2 = assembler.getsources(tks[3])
                noutputs = self.getnoutputs()
                self.code = assembler.create_opmask(INSTRSET[tks[0]], 2, noutputs)
                self.immed = self.tonum(tks[4])
                itype = self.getimmedtype()
                writetofile("I", [self.code], assembler.output)
                writetofile("%c"%itype, [self.immed], assembler.output)
                writetofile("I", [len(self.src1)] + self.src1 + [len(self.src2)] + self.src2, assembler.output)
        def getnoutputs(self):
                return 1
...
class specsuperi(SuperInstImmed):
        pass
class callsnd(ALU2i, IntIns):
        pass
class retsnd(ALU2i, IntIns):
        pass
class ret(ALU, IntIns):
        def getnoutputs(self):
                # Allow per-callgroup output ports (callgroup id is used as port index).
                return 31
