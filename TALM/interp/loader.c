...
void add_dst(instr_t **addrs, unsigned int index, int n, instr_t *dst) {
	int src_id = index >> SIZE_OF_SRC_OFFSET;
	int output = index & ((1 << SIZE_OF_SRC_OFFSET) - 1);
	if (src_id < 0 || src_id >= g_instr_count) {
		fprintf(stderr, "[loader] invalid src id=%d (instr_count=%d)\n",
		        src_id, g_instr_count);
		exit(1);
	}
	if (output < 0 || output >= MAX_DEST) {
		fprintf(stderr, "[loader] invalid src output=%d (MAX_DEST=%d)\n",
		        output, MAX_DEST);
		exit(1);
	}
	instr_t *src = addrs[src_id];
...
//	fread(&n, sizeof(int), 1, fp); //the number of instructions is in the header of the binary file
	for (i=0; i < n; i++) {
		...
		if (k >= args[placement[i]].instrs_cap) {
			fprintf(stderr, "Instruction buffer overflow on PE %d (cap=%d)\n",
			        placement[i], args[placement[i]].instrs_cap);
			exit(1);
		}
...
void loader(int placement[], thread_args *args, FILE *fp) {
	...
	if (getenv("DF_DEBUG_LOAD")) {
		fprintf(stderr, "[loader] header n=%d\n", n);
	}
	...
	for (i=0; fread(&opcode, sizeof(int), 1, fp); i++) {
		...
		instr->n_src = n_src;	
		instr->n_dst = n_dst;
		if (getenv("DF_DEBUG_INSTMAP")) {
			fprintf(stderr, "[inst ] id=%d op=%d n_src=%d n_dst=%d\n",
		...
		for (j=0; j < n_src; j++) {
			...
			if (getenv("DF_DEBUG_SRC_ID")) {
				int dbg = atoi(getenv("DF_DEBUG_SRC_ID"));
				if (dbg == instr->id) {
					fprintf(stderr, "[src ] id=%d op=%d port=%d srclen=%d\n",
					        instr->id, instr->opcode, j, srclen);
				}
			}
			#ifdef DEBUG_LOADER
			printf("src_len = %d\n", srclen);
