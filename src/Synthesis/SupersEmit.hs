...
emitSupersModule baseName specs =
      let specs' = filter (\(SuperSpec nm _ _) -> nm `notElem` ["s0","s1","s2","s3","s4","s5","s6","s7","s8","s9"]) specs
      in unlines $
      [ "{-# LANGUAGE ForeignFunctionInterface #-}"
      ...
      , "  poke pout v"
      , ""
      , "-- s6: prints — list of ASCII codes as string"
      , "foreign export ccall \"s6\" s6 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s6 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s6 pin pout = do"
      , "  v <- peek pin"
      , "  if v == handleNil then putStrLn \"\""
      , "  else putStrLn (map (toEnum . fromIntegral) (toList v) :: String)"
      , "  hFlush stdout"
      , "  poke pout v"
      , ""
      , "-- s7: printf — single float"
      ...
      , "  poke pout v"
      , ""
      , "-- s8: printlf — list of floats"
      , "foreign export ccall \"s8\" s8 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s8 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s8 pin pout = do"
      , "  v <- peek pin"
      , "  if v == handleNil then putStrLn \"[]\""
      , "  else print (toListF v)"
      , "  hFlush stdout"
      , "  poke pout v"
      , ""
      , "-- s9: printmf — matrix (list of list of floats)"
      , "foreign export ccall \"s9\" s9 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s9 :: Ptr Int64 -> Ptr Int64 -> IO ()"
      , "s9 pin pout = do"
      , "  v <- peek pin"
      , "  if v == handleNil then putStrLn \"[]\""
      , "  else print (map toListF (toList v))"
      , "  hFlush stdout"
      , "  poke pout v"
      ]
