#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate N-Queens .hs using Control.Parallel.Strategies.
...
-- Auto-generated: N-Queens counting (GHC Strategies)
...
nqueens !n !cutoff !row !cols
  ...
  | otherwise     =
      let candidates = [ col | col <- [0..n-1], isSafe cols row col ]
          results = parMap rdeepseq
                      (\\col -> nqueens n cutoff (row + 1) (packQueen cols row col))
                      candidates
      in sum results
...
def main():
    ...
    print(f"[gen_nq_strategies] wrote {args.out} (N={args.N}, cutoff={args.cutoff})")
