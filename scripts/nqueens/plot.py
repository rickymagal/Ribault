#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Plot N-Queens benchmark results: runtime, speedup, efficiency.

Reads one or more CSV files (variant column: super, ghc, parpseq).
CSV format: variant,N,P,rep,seconds,rc

For each variant:
  - runtime vs N (curves per P)
  - speedup vs P (curves per N)

When multiple variants are present:
  - comparison: best configuration runtime vs N
  - comparison: best speedup vs N
"""
...
def plot_runtime(byN, Ns, Ps, outdir, tag, variant):
    ...
    plt.title(f"N-Queens ({label}): Runtime vs N", fontsize=12)
        ...
        for P in Ps:
            ...
            if p_vals:
                ...
                if mp > 0 and mb > 0:
                    ...
                    sus.append(mb / mp)
        if ps:
            plt.plot(ps, sus, marker="o", label=f"N={N}")
    # Ideal line
    plt.plot(Ps, Ps, "k--", alpha=0.3, label="Ideal")
    label = VARIANT_LABELS.get(variant, variant)
    plt.title(f"N-Queens ({label}): Speedup vs P (baseline P={baseP})", fontsize=12)
    plt.xlabel("Processors", fontsize=11)
...
def plot_compare_runtime(data, Ns, outdir, tag):
    ...
    for variant in ("super", "ghc", "parpseq"):
        ...
        si += 1
    plt.title("N-Queens: Best Configuration (Runtime vs N)", fontsize=12)
    plt.xlabel("N (board size)", fontsize=11)
    plt.ylabel("Runtime (seconds)", fontsize=11)
    plt.grid(True, ls=":", lw=0.8)
    plt.legend(fontsize=9)
...
def plot_compare_speedup(data, Ns, outdir, tag):
    ...
    for variant in ("super", "ghc", "parpseq"):
        ...
        si += 1
    plt.title("N-Queens: Best Speedup vs N", fontsize=12)
    plt.xlabel("N (board size)", fontsize=11)
