...
# ============================================================
# Validated MatMul benchmark with file-IO data pipeline
#
# Each parallel block reads A.bin + BT.bin via BS.readFile
# (GHC heap allocation), then computes its row-block of C = A*B^T.
#
...
#
# Structure:
#   - Pre-generate binary matrices per N
...
GEN_MATRICES="$REPO/scripts/matmul/gen_matrices.py"
GEN_TALM="$REPO/scripts/matmul/gen_talm_input_io.py"
GEN_SEQ="$REPO/scripts/matmul/gen_hs_sequential_io.py"
GEN_STRAT="$REPO/scripts/matmul/gen_hs_strategies_io.py"
GEN_PARPSEQ="$REPO/scripts/matmul/gen_hs_parpseq_io.py"
...
NS=(${NS:-4096 5120 6400 7680 8704})
...
echo "========================================="
echo " MatMul Data Pipeline Benchmark"
echo " N values: ${NS[*]}"
...
for N in "${NS[@]}"; do
  ...
  DATADIR="$NDIR/data"
  if [[ ! -f "$DATADIR/A.bin" || ! -f "$DATADIR/BT.bin" ]]; then
    "$PY3" "$GEN_MATRICES" --out-dir "$DATADIR" --N "$N"
  else
    echo "  [gen_matrices] reusing existing $DATADIR/{A,BT}.bin"
  fi

  # --- Build TALM ---
  TDIR="$NDIR/talm"
  mkdir -p "$TDIR/supers"
  "$PY3" "$GEN_TALM" --out "$TDIR/mm.hsk" --N "$N" --n-funcs "$N_FUNCS" --data-dir "$DATADIR"
  ...
  CFLAGS="$SUPERS_CFLAGS" SUPERS_INJECT_FILE="$TDIR/supers_inject.hs" \
    ...
    bash "$BUILD_SUPERS" "$TDIR/mm.hsk" "$TDIR/supers/Supers.hs"
  ...
  SDIR="$NDIR/seq"
  mkdir -p "$SDIR/obj"
  "$PY3" "$GEN_SEQ" --out "$SDIR/mm.hs" --N "$N" --n-funcs "$N_FUNCS" --data-dir "$DATADIR"
  ...
  GDIR="$NDIR/ghc"
  mkdir -p "$GDIR/obj"
  "$PY3" "$GEN_STRAT" --out "$GDIR/mm.hs" --N "$N" --n-funcs "$N_FUNCS" --data-dir "$DATADIR"
  ...
  PDIR="$NDIR/parpseq"
  mkdir -p "$PDIR/obj"
  "$PY3" "$GEN_PARPSEQ" --out "$PDIR/mm.hs" --N "$N" --n-funcs "$N_FUNCS" --data-dir "$DATADIR"
  "$GHC_BIN" -O2 -threaded -rtsopts -package time -package bytestring \
  ...
  for ((w=1; w<=WARMUP; w++)); do
    ...
    taskset -c "$CORES_1" "$SDIR/mm" +RTS -N1 -RTS >/dev/null 2>/dev/null
  done

  for ((rep=1; rep<=REPS; rep++)); do
    OUT="$SDIR/out_r${rep}.txt"
    taskset -c "$CORES_1" "$SDIR/mm" +RTS -N1 -RTS >"$OUT" 2>/dev/null
  ...
  for P in "${PS[@]}"; do
    ...
    for ((w=1; w<=WARMUP; w++)); do
      ...
      taskset -c "$CORES_P" "$GDIR/mm" +RTS -N"$P" -RTS >/dev/null 2>/dev/null
    done

    for ((rep=1; rep<=REPS; rep++)); do
      OUT="$GDIR/out_P${P}_r${rep}.txt"
      taskset -c "$CORES_P" "$GDIR/mm" +RTS -N"$P" -RTS >"$OUT" 2>/dev/null
      secs="$(awk -F= '/^RUNTIME_SEC=/{print $2}' "$OUT")"
    ...
    for ((w=1; w<=WARMUP; w++)); do
      ...
      taskset -c "$CORES_P" "$PDIR/mm" +RTS -N"$P" -RTS >/dev/null 2>/dev/null
    done

    for ((rep=1; rep<=REPS; rep++)); do
      OUT="$PDIR/out_P${P}_r${rep}.txt"
      taskset -c "$CORES_P" "$PDIR/mm" +RTS -N"$P" -RTS >"$OUT" 2>/dev/null
      secs="$(awk -F= '/^RUNTIME_SEC=/{print $2}' "$OUT")"
