#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate matrix multiplication .hs using only par/pseq (NO Strategies)."""

import argparse, os

TMPL = r"""{-# LANGUAGE BangPatterns #-}
-- Auto-generated: Matrix Multiply (par/pseq, NO Strategies)
-- N=__N__
...
parMapRows :: NFData b => (a -> b) -> [a] -> [b]
parMapRows _ [] = []
parMapRows f (x:xs) =
  let fx   = force (f x)
      rest = parMapRows f xs
  in fx `par` rest `pseq` (fx : rest)

matMul :: Matrix -> Matrix -> Matrix
matMul a b =
  let bt = transpose' b
  in parMapRows (\row -> map (dot row) bt) a
...
def main():
    ...
    with open(args.out, "w") as f:
        f.write(src)
    print(f"[gen_matmul_parpseq] wrote {args.out} (N={args.N})")
