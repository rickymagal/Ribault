#!/usr/bin/env python3
"""Generate GHC par/pseq matmul .hs that reads matrices from binary files.
...
-- Auto-generated: MatMul GHC par/pseq (file IO via BS.readFile)
...
main = do
  ...
  let ranges = {range_list}
      !results = parMapPP evalBlock ranges
      !total = sum results
  t1 <- getCurrentTime
...
"""
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"[gen_matmul_pp_io] wrote {path} (N={N}, n_funcs={len(ranges)})")
