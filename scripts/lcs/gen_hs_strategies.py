#!/usr/bin/env python3
"""Generate GHC Strategies .hs for LCS benchmark.

Same algorithm as TALM version: compute LCS for N_PAIRS independent
string pairs, sum all lengths.  Uses parMap + rseq for parallelism.
"""

import argparse, os

HS_TMPL = r"""-- Auto-generated: LCS benchmark (GHC Strategies)
...
main = do
  t0 <- getCurrentTime
  let chunks = parMap rseq computeChunk [0 .. lcsNFuncs - 1]
      !total = sum chunks
  t1 <- getCurrentTime
...
def emit_hs(path, input_dir, n_funcs):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"[gen_lcs_ghc] wrote {path}  (n_pairs={n_pairs}, n_funcs={n_funcs})")
