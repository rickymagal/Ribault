#!/usr/bin/env python3
"""Generate a GHC Strategies 0/1 knapsack brute-force benchmark (standalone .hs).
...
GHC Strategies (parMap rdeepseq).  Uses identical list-based algorithm.
"""
...
-- Auto-generated: 0/1 Knapsack brute-force (GHC Strategies)
...
main = do
  ...
  let chunks = [0 .. ksNFuncs - 1]
      results = parMap rdeepseq ksSearchChunk chunks
...
"""
    with open(path, "w", encoding="utf-8") as f:
        f.write(hs)
    print(f"[gen_ks_strategies] wrote {path} (n_items={n_items}, n_funcs={n_funcs})")
