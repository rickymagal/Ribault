#!/usr/bin/env python3
"""Generate GHC par/pseq .hs for 0/1 Knapsack via parallel merge-sort aggregation."""

import argparse, os

HS_TMPL = r"""-- Auto-generated: 0/1 Knapsack via parallel merge sort (par/pseq)
...
-- In-place mergesort on STUArray (DESCENDING)
...
-- Merge two sorted (descending) UArrays
...
-- Parallel mergesort with par/pseq (DESCENDING)
...
main = do
  ...
  let secs    = realToFrac (diffUTCTime t1 t0) :: Double
      maxVal  = if n > 0 then ys ! 0 else 0
...
def emit_hs(path, items_dir, n_items, nparts):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"[gen_ks_ms_parpseq] wrote {path}")
