#!/usr/bin/env python3
"""Generate a GHC Strategies text-search benchmark (standalone .hs).
...
main = do
  ...
  let ranges = {range_list}
      results = parMap rdeepseq processRange ranges
      !total = sum results
  t1 <- getCurrentTime
...
"""
    with open(path, "w", encoding="utf-8") as f:
        f.write(hs)
    print(f"[gen_ts_strategies] wrote {path} (n_files={n_files}, n_funcs={len(ranges)})")
