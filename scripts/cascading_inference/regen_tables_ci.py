#!/usr/bin/env python3
"""Regenerate tables_with_ci.txt for the Cascading Inference Pipeline
benchmark with per-language baselines and 95% bootstrap CIs.

CSV schema: variant,N,chunk_size,P,rep,seconds,checksum,expected
"""
import argparse, csv, os, random, statistics, sys
from collections import defaultdict

DEFAULT_CSV = "results/cascading_inference_paper_final/master.csv"
DEFAULT_OUT = "results/cascading_inference_paper_final/tables_with_ci.txt"
...
def main():
    ...
    out = []
    out.append(f"CASCADING INFERENCE PIPELINE — median speedup with 95% bootstrap CI ({B} independent resamples)")
    ...
    out.append("  C tier       (Ribault-C)                            -> seq_c        (gcc -O3 -march=native, STRICT IEEE)")
    out.append("  Rust tier    (Ribault-Rust, Timely, Sucuri)         -> seq_rust     (rustc release, raw-ptr unsafe)")
    out.append("")
    ...
    out.append("=" * 60); out.append("")
    for N in Ns:
        rh = data.get("ribault_rust", {}).get(N, {})
        st = data.get("timely", {}).get(N, {})
        if not rh or not st: continue
        out.append(f"=== N={N} ===")
