...
-- Flat parallel map using par/pseq
parMapPP :: NFData b => (a -> b) -> [a] -> [b]
parMapPP _ [] = []
parMapPP f (x:xs) =
  let r = f x
      rs = parMapPP f xs
  in rnf r `par` (rnf rs `pseq` (r : rs))

mergeResults :: [(Int, Int)] -> (Int, Int)
mergeResults rs = (maximum (map fst rs), sum (map snd rs))

main :: IO ()
main = do
  t0 <- getCurrentTime
  let results = parMapPP colorChunk chunks
      !(maxColor, nColored) = mergeResults results
...
"""
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"[gen_hs_parpseq] wrote {path} (N={n}, P={p}, edge_prob={edge_prob})")
