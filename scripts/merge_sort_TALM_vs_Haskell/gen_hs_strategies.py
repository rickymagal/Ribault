...
mergeSortStrat :: Ptr Int -> Ptr Int -> Int -> Int -> Int -> Int -> ()
mergeSortStrat arr tmp n0 p lo hi
  | hi - lo <= 1 = ()
  | hi - lo <= n0 `div` p = unsafePerformIO (mergeSortSeq arr tmp lo hi)
  | otherwise = unsafePerformIO $ do
      let mid = lo + (hi - lo) `div` 2
          !_ = runEval $ do
                 sl <- rpar (mergeSortStrat arr tmp n0 p lo mid)
                 sr <- rseq (mergeSortStrat arr tmp n0 p mid hi)
                 _ <- rseq sl
                 return sr
      mergeArr arr tmp lo mid hi
...
main = do
  ...
  fill 0
  t0 <- getCurrentTime
  let !_ = mergeSortStrat arr tmp n parP 0 n
