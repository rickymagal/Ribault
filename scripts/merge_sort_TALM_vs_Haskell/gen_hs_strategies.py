#!/usr/bin/env python3
"""Generate GHC Strategies Haskell merge sort (array-based) for benchmark.

Same algorithm as all other variants: top-down merge sort on flat C array.
Uses Control.Parallel.Strategies (Eval monad, rpar/rseq).
...
{-# NOINLINE mergeSortStrat #-}
mergeSortStrat :: Ptr Int -> Ptr Int -> Int -> Int -> Int -> ()
mergeSortStrat arr tmp n0 lo hi
  | hi - lo <= 1 = ()
  | hi - lo <= n0 `div` parCutoff = unsafePerformIO (mergeSortSeq arr tmp lo hi)
  | otherwise = unsafePerformIO $ do
      let mid = lo + (hi - lo) `div` 2
          !_ = runEval $ do
                 sl <- rpar (mergeSortStrat arr tmp n0 lo mid)
                 sr <- rseq (mergeSortStrat arr tmp n0 mid hi)
                 _ <- rseq sl
                 return sr
...
main = do
  ...
  fill 0
  t0 <- getCurrentTime
  let !_ = mergeSortStrat arr tmp n 0 n
...
def emit_hs(path, n, p, g):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(src)
    print(f"[gen_ms_strat] wrote {path}  (N={n}, P={p}, G={g}, cutoff=N/{cutoff})")
