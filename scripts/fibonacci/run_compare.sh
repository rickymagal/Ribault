...
# ============================================================
# fibonacci/run_compare.sh — TALM + GHC-strategies + GHC-par/pseq
# ============================================================

N_CSV="30,35,40,45"; CUTOFF_CSV="15,20,25"; REPS=10
PROCS_CSV="1,2,4,8"
...
if [[ "$SKIP_TALM" -eq 1 && -f "$TALM_CSV" ]]; then
  ...
  bash "$SCRIPT_DIR/run_talm.sh" \
    --N "$N_CSV" --cutoff "$CUTOFF_CSV" --reps "$REPS" --procs "$PROCS_CSV" \
